...
In \SBGNERLone, \glyph{state variables} are also used to describe the localisation in compartments (a transport \corr{is therefore could}{could therefore} \rougny{or ``is therefore described'' if it is the only way to describe one} be described as assignment of different values to the location state variable, see \sect{assignment}).
