...
\noindent
An \glyph{assignment} (\sect{assignment}) linking a \glyph{variable value} $v$ to a \glyph{state variable} $V$ of an \glyph{entity} $E$ means: ``$v$ is assigned to $V$ of $E$'' or ``$V$ of $E$ takes the value $v$''. An \glyph{outcome} on an \glyph{assignment} represents the cases when the statement is true, that is all instances of $E$ (free or as a complex \corr{subunint}{subunit}) on which the variable effectively displays the value. It is used as follows: ``when (or if) $V$ of $E$ takes the value $v$ then \ldots'' or more succintly ``when (or if) $E\{V => v\}$ then \ldots''.\\[\baselineskip]
