...
SBGN \ERs{} describe biological interactions involving biological entities. An \glyph{entity node} (\sect{ENs}), such as a molecule, influences the behaviour of other \glyph{entities} via a relationship. 

It may be convenient to think of a SBGN \ERs{} as listing independent rules that describe influences between \glyph{entities}. A map can then be analysed with ``what if?'' queries. 
