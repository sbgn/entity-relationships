...
Nodes are only allowed to overlap in the case that the overlapping nodes define a glyph. Examples are stacking auxiliary items such as an \glyph{unit of information} on top of an \glyph{entity}, or an \glyph{interaction}. In other cases, nodes are not allowed to overlap (\fig{layout1}). This includes the touching of nodes. 
