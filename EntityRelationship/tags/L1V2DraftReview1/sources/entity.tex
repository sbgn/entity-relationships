...
\begin{figure}[H]
  ...
  \caption{Example of an \glyph{entity} named CaMKII, that carries two \glyph{state variables} representing the phosphorylated residue threonine 286, and the residu threonine 306, a \glyph{unit of information} precising its material status (protein).}
  \label{fig:ex-entity}
\end{figure}

The granularity of the representation, that is what a given \glyph{entity} actually represents, is let to the person generating the map. For instance (\fig{entity-granularity}), one could choose to represent phosphorylated and non-phosphorylated forms of a protein using a single \glyph{entity}, carrying a \glyph{state variable} representing the phosphorylation. The phosphorylated form would be represented by the \glyph{outcome} located on the \glyph{assignment}. Alternatively one could use two separate entities, carrying the \glyph{existence} \glyph{state variable}. The phosphorylated form would be represented by the \glyph{entity}.
