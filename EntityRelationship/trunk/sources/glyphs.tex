% =============================================================================
% glyphs
% =============================================================================

\chapter{Entity Relationship Glyphs}

This chapter provides a catalog of the graphical symbols available for representing entities in \ER diagrams.  There are different classes of glyphs corresponding to different classes of entities, predicates, controls and operators.

In \chap{grammar} beginning on page~\pageref{chp:grammar}, we describe the rules for combining these glyphs into a legal SBGN \ER, and in \chap{layout} beginning on page~\pageref{chp:layout}, we describe requirements and guidelines for the way that diagrams are visually organized.

\section{Overview}
 
\input{sources/er-overview.tex}
 
\section{Controlled vocabularies used in \SBGNERLone}\label{sec:CVs}

\input{sources/CVs.tex}
 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%%%                   Entity nodes
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 \section{Interactor nodes}\label{sec:interactors}

\SBGNERLone{} contains three glyphs representing classes of interactors: \glyph{entity}, \glyph{perturbing agent} and \glyph{outcome}. Entities can optionally carry auxiliary units such as \glyph{units of information} and \glyph{state variables}.

\input{sources/entity.tex}
\input{sources/perturbation.tex}
\input{sources/outcome.tex}

\section{Auxiliary units}\label{sec:aux}

\input{sources/unitInformation.tex}
\input{sources/stateVariable.tex}
\input{sources/domain.tex}
\input{sources/submap.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%                   Relationship arcs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Statements}\label{sec:statements}

\input{sources/assignment.tex}
\input{sources/interaction.tex}
\input{sources/observable.tex}

\section{influences}\label{sec:influences}

Influence arcs represent the effect of an entity on another relationship. The symbols attached to their extremities precise their semantics. \SBGNERLone's influences can be viewed as logical rules linking interactors and other rules.

\input{sources/modulation.tex}
\input{sources/stimulation.tex}
\input{sources/inhibition.tex}
\input{sources/necessaryStimulation.tex}
\input{sources/absoluteInhibition.tex}
\input{sources/logicArc.tex}

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%                   Logical operators
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Logical operators}\label{sec:logic}
 
\input{sources/and.tex}
\input{sources/or.tex}
\input{sources/not.tex}
\input{sources/delay.tex}

% The following is for [X]Emacs users.  Please leave in place.
% Local Variables:
% TeX-master: "../sbgn_ER-level1"
% End:
