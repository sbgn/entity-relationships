...
\item There cannot be both an \glyph{absoluteStimulation} and an \glyph{absoluteInhibition} targeting the same statement.
...
\item If more than one instance of an \glyph{entity} is involved in an \glyph{interaction} or a \glyph{non-interaction}, a \glyph{unit of information} \glyph{cardinality} (\sect{miscellaneous-cv}) must be associated with each entity involved in the statement.
\item A \glyph{cis} or \glyph{trans} unit of information can only be carried by a relationship involving a single \glyph{entity}.
\end{enumerate}  
